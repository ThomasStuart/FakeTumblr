...
class PhotosViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    // create an array of dictionaries to
        ...
        let task = session.dataTask(with: url) { (data, response, error) in
            if let error = error {
               ...
               // Get the dictionary from the response key
                ...
                self.tableView.reloadData() // <==  Call this method to reload all the data that is used to construct the table
                //...including cells, section headers and footers, index arrays, and so on.
